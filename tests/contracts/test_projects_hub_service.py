...
def test_projects_service_crud_and_cas():
    ...
    # 5. 归档与解归档
    arch_res = service.archive_project(new_pid, expected_version=2)
    assert arch_res.version == 3
    assert arch_res.archived_at is not None

    unarch_res = service.unarchive_project(new_pid, expected_version=3)
    assert unarch_res.version == 4
    assert unarch_res.archived_at is None

    # 6. 回收站流转
    trash_res = service.move_to_trash(new_pid, expected_version=4)
    assert trash_res.version == 5
    ...
    # 恢复
    restore_res = service.restore_from_trash(new_pid, expected_version=5)
    assert restore_res.version == 6
...
def test_api_static_and_projects_integration(client: TestClient):
    ...
    assert "洁净重构中" in resp_workshop.text
