...
function smartCanvasApi(){
    const api = window.GodsWorkbenchSmartCanvasApi;
    if(!api) throw new Error('Smart canvas HTTP facade is unavailable');
    return api;
}

const sourceProjectId = params.get('project_id') || params.get('project') || '';
function openSmartProxySettings(){
...
window.addEventListener('storage', event => {
    if(event.key !== PROMPT_SOURCE_SETTINGS_KEY || !promptLibraries.length) return;
    smartCanvasApi().getPromptLibraries().then(async response => {
...
function snapshotForUndo(){
    return {
        ...
        deletedNodeIds:[...localDeletedNodeIds],
        unsyncedNodeIds:[...localUnsyncedNodeIds],
        selectedId,
...
function performUndo(){
    ...
    (snap.deletedNodeIds || []).forEach(id => localDeletedNodeIds.add(id));
    localUnsyncedNodeIds.clear();
    (snap.unsyncedNodeIds || []).forEach(id => localUnsyncedNodeIds.add(id));
...
function downloadBlob(blob, filename, options={}){
    return window.GodsWorkbenchSmartCanvasMedia.downloadBlob(blob, filename, options);
}
function smartWorkflowFilename(ext='json'){
    const title = (canvas?.title || 'smart-canvas').trim();
...
async function exportSelectedSmartWorkflow(includeResources=false){
    ...
    try {
        ...
        const res = await smartCanvasApi().exportCanvasWorkflow({...payload, include_resources:true, filename});
...
async function importSmartWorkflowFile(file){
    ...
    try {
        ...
        const res = await smartCanvasApi().importCanvasWorkflow(form);
...
function notifySmartCanvasContext(title=canvas?.title){
...
function isEditableTarget(target){
    return window.GodsWorkbenchSmartCanvasInteraction.isEditableTarget(target);
}
function safeScale(value){
    return window.GodsWorkbenchSmartCanvasRender.safeScale(value);
}
...
function applyViewport(){
    return window.GodsWorkbenchSmartCanvasRender.applyViewport(world, shell, viewport, renderMinimap, scheduleSmartImageResolutionSync);
}
function screenToWorld(event){
    return window.GodsWorkbenchSmartCanvasRender.screenToWorld(shell, viewport, event);
}
function canvasWheelZoomFactor(event, pageSize){
    return window.GodsWorkbenchSmartCanvasRender.canvasWheelZoomFactor(event, pageSize);
}
function viewportCenter(){
    return window.GodsWorkbenchSmartCanvasRender.viewportCenter(shell, viewport);
}
...
async function ensureComfyWorkflow(name){
    ...
    const load = (async () => {
        const data = await smartCanvasApi().getWorkflow(name).then(r => r.ok ? r.json() : null).catch(() => null);
...
function renderRunningHubParams(){
    ...
    if(ref.kind === 'workflow' && !fields.length && !runningHubWorkflowCache[ref.id] && !runningHubWorkflowLoads[ref.id]){
        ensureRunningHubWorkflow(ref.id).then(() => renderDynamicParams({force:true})).catch(() => {});
    }
    if(ref.kind === 'model'){
...
function renderComfyParams(){
    ...
    if(settings.comfyMode === 'custom'){
        ...
        if(settings.comfyWorkflow && !comfyWorkflowCache[settings.comfyWorkflow]) ensureComfyWorkflow(settings.comfyWorkflow).then(() => renderDynamicParams({force:true}));
    }
    let html = '';
...
async function ensureRunningHubWorkflow(workflowId){
    ...
    const load = (async () => {
        const res = await smartCanvasApi().getRunningHubWorkflow(workflowId);
        const loadRemoteInfo = async saved => {
            try {
                const infoRes = await smartCanvasApi().getRunningHubWorkflowInfo(workflowId);
...
async function uploadMediaRefToCloud(ref){
    ...
    const response = await smartCanvasApi().uploadCloudVideo({url:sourceUrl, service:'auto'});
...
async function rhUploadValueIfNeeded(value, sourceSettings=settings){
    ...
    const res = await smartCanvasApi().uploadRunningHubAsset({url:text, useWallet:(sourceSettings || settings).rhPayment === 'wallet'});
    const data = await res.json();
...
function rhInputBindingSnapshot(fields, nodeInfoList, media){
    ...
    return (fields || []).filter(field => ['image','video','audio'].includes(rhFieldKind(field))).map(field => {
        ...
        return {
            ...
            submittedFileName:displayName(item.fieldValue),
...
function setDynamicSetting(key, value){
    ...
    if(key === 'comfyWorkflow') {
        ...
        ensureComfyWorkflow(settings.comfyWorkflow).then(() => renderDynamicParams({force:true}));
    }
    if(key === 'rhConfigKey'){
        settings.rhParams = {};
        settings.rhRandomActive = {};
        const ref = selectedRunningHubRef();
        if(ref?.kind === 'workflow') ensureRunningHubWorkflow(ref.id).then(() => renderDynamicParams({force:true})).catch(() => {});
    }
    persistActiveSmartSettings();
...
async function loadConfig(options={}){
    ...
    try {
        const cfg = await smartCanvasApi().getConfig().then(r => r.json());
        ...
        const wf = await smartCanvasApi().getWorkflows().then(r => r.json()).catch(() => ({workflows:[]}));
...
async function loadCanvas(){
    ...
    try {
        ...
        const data = await res.json();
        canvas = data.canvas;
        smartCanvasConflictPending = false;
        smartCanvasDirty = false;
        rememberCanvasListProject(canvas.project || 'default');
        syncSmartCanvasRouteContext();
        notifySmartCanvasContext();
...
async function saveCanvas(){
    if(!canvasId || !canvas || smartCanvasConflictPending) return;
    smartCanvasDirty = true;
    savePromptDraftForCurrent();
    nodes.forEach(node => {
    ...
    try {
        const res = await smartCanvasApi().saveCanvas(canvasId, {
                ...
                expected_version:Number(storageCanvas.governance_version || canvas.governance_version || 0),
        ...
        if(res.ok){
            ...
            enterSmartCanvasConflictPending(data);
        }
    } catch(e) {} finally {
...
function resultMediaUrls(result){
    ...
    const add = value => {
        ...
        if(typeof value === 'object'){
            ...
            const assetId = typeof value.asset_id === 'string' ? value.asset_id.trim() : (typeof value.assetId === 'string' ? value.assetId.trim() : '');
...
function safeExportFileName(name, fallback='download.zip'){
    return window.GodsWorkbenchSmartCanvasUtils.safeExportFileName(name, fallback);
}
...
async function zipDownloadImageItems(title, items){
    ...
    try {
        ...
        const response = await smartCanvasApi().downloadCanvasAssets({
            filename,
...
function smartRunRequestMeta(run){
    const s = run?.settings || {};
    const extra = run?.request && typeof run.request === 'object' ? run.request : {};
    if(s.engine === 'comfy') return {...{workflow_json:s.comfyWorkflow || '', mode:s.comfyMode || 'text'}, ...extra};
    if(s.engine === 'modelscope') return {...{backend:'Modelscope', model:s.msgenModel || '', custom_model:s.msCustomModel || ''}, ...extra};
    if(run?.kind === 'video') return {...{provider_id:s.videoProvider || '', model:s.videoModel || '', duration:s.videoDuration || '', aspect_ratio:s.videoAspect || '', resolution:s.videoResolution || ''}, ...extra};
    return {...{provider_id:s.provider_id || '', model:s.model || '', size:run?.size || '', quality:s.quality || '', n:s.count || 1}, ...extra};
}
...
function openSmartLogLightbox(url, kind='image'){
    ...
    if(!box){
        ...
        box.innerHTML = `<img data-floating-content alt="preview" draggable="false"><button class="smart-log-lightbox-close" type="button" aria-label="${escapeAttr(tr('common.close') || '关闭')}"><i data-lucide="x"></i></button>`;
        document.body.appendChild(box);
        window.FloatingDismissal?.mark(box, {
            surface: 'smart-log-lightbox',
            kind: 'modal',
            content: 'img',
            close: closeSmartLogLightbox,
        });
...
async function deleteCanvasLogEntry(logId, deleteMedia=false){
    ...
    if(smartCanvasConflictPending){
        toast('保存冲突：请刷新画布后再继续编辑。');
        return;
    }
    ...
    try {
        ...
        const res = await smartCanvasApi().deleteCanvasLog(canvasId, {
                log_id:logId,
...
function smartCanvasImageTaskProductionContext(){
    return {
...
const SMART_STABLE_ASSET_ID_PATTERN = /^[A-Za-z0-9][A-Za-z0-9._:-]{0,159}$/;
function smartStableAssetId(item){
    const assetId = typeof item?.asset_id === 'string' ? item.asset_id.trim() : '';
    return SMART_STABLE_ASSET_ID_PATTERN.test(assetId) ? assetId : '';
}
function smartInputAssetIds(refs=[]){
    const ids = [];
    const seen = new Set();
    (Array.isArray(refs) ? refs : []).forEach(ref => {
        if(ids.length >= 32) return;
        const assetId = smartStableAssetId(ref);
...
function withSmartInputAssetIds(payload={}, refs=[]){
    return {...payload, input_asset_ids:smartInputAssetIds(refs)};
}

function syncSmartCanvasRouteContext(){
    const projectId = String(canvas?.project || '').trim();
    const entityId = String(canvas?.entity_id || '').trim();
    const stableCanvasId = String(canvas?.id || canvasId || '').trim();
    if(!projectId || !stableCanvasId) return;
    const url = new URL(window.location.href);
    url.searchParams.set('project_id', projectId);
    url.searchParams.set('canvas_id', stableCanvasId);
...
async function runJimengUpscale(node, index){
    ...
    try {
        const payload = {
            ...
            input_asset_ids:smartInputAssetIds([item]),
            production_context:smartCanvasImageTaskProductionContext()
        };
        const task = await smartCanvasApi().createCanvasImageTask(payload).then(async r => {
...
function formatRunDuration(ms){
    return window.GodsWorkbenchSmartCanvasUtils.formatRunDuration(ms);
}
...
async function ensurePanoramaRenderer(){
    ...
    if(!panoramaState.three){
        panoramaState.threeLoadPromise = panoramaState.threeLoadPromise || import('/static/vendor/js/three-0.160.0.module.js?v=v0.0.1-alpha-2026');
...
async function uploadCroppedBlob(blob, name){
    ...
    const data = await smartCanvasApi().uploadAiReferences(form).then(r => r.json());
    return data.files?.[0];
}
...
async function smartResponseErrorMessage(response, fallback='请求失败'){
    return responseErrorMessage(response, fallback);
}
...
async function uploadFiles(files){
    ...
    const data = await smartCanvasApi().uploadAiReferences(form).then(async r => {
...
async function importSmartLocalImages(paths){
    ...
    const response = await smartCanvasApi().importLocalImagePaths({paths:(paths || []).slice(0, SMART_UPLOAD_MAX)});
...
window.addEventListener('keydown', e => {
    ...
    if((e.key === 'Escape' || e.key === '·') && !e.isComposing && !e.ctrlKey && !e.metaKey && !e.altKey
        ...
        && !assetDialogBackdrop?.classList.contains('open')
...
window.GodsWorkbenchSmartCanvasInteraction.bindKeyboardLifecycle({
...
if(assetAddCategoryBtn) assetAddCategoryBtn.onclick = async () => {
    ...
    if(assetLibraryIsLocal()){
        const data = await smartCanvasApi().createLocalAssetFolder({parent:localAssetFolderPath(), name}).then(async r => {
    ...
    const data = await smartCanvasApi().createAssetLibraryCategory({library_id:activeAssetLibraryId, name, type:workflowMode ? 'workflow' : 'image'}).then(r => r.json());
...
if(assetRenameCategoryBtn) assetRenameCategoryBtn.onclick = async () => {
    ...
    if(!name) return;
    if(assetLibraryIsLocal()){
        const data = await smartCanvasApi().renameLocalAssetFolder({path:cat.id || '', name}).then(async r => {
    ...
    const data = await smartCanvasApi().renameAssetLibraryCategory(cat.id, {name}).then(r => r.json());
    setAssetLibraryFromResponse(data);
};
        ...
        try {
            if(raw){
                ...
                if(/^https?:\/\//i.test(String(payload?.url || '').trim())){
                    await saveUrlsToAssetUrlLibrary([payload.url], [payload.name || smartImageNameFromUrl(payload.url)]);
                    toast('已保存 URL 素材');
                } else {
                    toast('仅支持公网 http(s) URL，本地素材请保存到图片资产库');
                }
                return;
            }
            const payload = await resolveSmartImageDropPayload(e.dataTransfer);
            if(payload.type === 'url' && /^https?:\/\//i.test(String(payload.url || '').trim())){
                await saveUrlsToAssetUrlLibrary([payload.url], [smartImageNameFromUrl(payload.url)]);
                toast('已保存 URL 素材');
            } else if(payload.type === 'files' || payload.type === 'localPaths') {
                toast('本地文件不能注册为公网 URL，请使用图片资产库上传');
            } else {
                toast('请拖入有效的 http(s) URL');
            }
        } catch(err) {
...
document.addEventListener('keydown', event => {
    ...
    const handled = Boolean(
        ...
        || smartWorkflowTransferModal?.classList.contains('open')
        || assetDialogBackdrop?.classList.contains('open')
        || createMenu?.classList.contains('open')
        || promptPresetPanel?.classList.contains('open')
        || promptTemplatePanel?.classList.contains('open')
        || document.querySelector('.smart-control.pinned, .smart-control.interacting')
        || document.getElementById('smartLogLightbox')?.classList.contains('open')
    );
...
window.GodsWorkbenchSmartCanvasInteraction.bindThemeLifecycle({applyTheme});
try {
...
window.onload = async () => {
    applyTheme(localStorage.getItem('studio_theme') || localStorage.getItem('canvas_theme') || localStorage.getItem('theme') || 'dark');
    loadPromptPresets();
