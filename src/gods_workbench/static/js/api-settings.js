let providers = [];
let selectedId = '';
let rhPasteMeta = { value:'', title:'' };
const rhTitleHydrationIds = new Set();
const rhTitleHydrationAttemptedIds = new Set();
...
let rhWorkflowEditorState = { open:false, index:-1, entry:null, config:null, expanded:{}, activeNodeId:'', graph:{ k:1, x:0, y:0, w:0, h:0 }, pan:null, bound:false, previewParams:{}, previewRunning:false, previewStatus:'', previewOutputs:[], previewJobId:'', previewAssetIds:[] };
...
function rhEntryFallbackTitle(kind, id){
    const cleanId = String(id || '').trim();
    return kind === 'app' ? `AI 应用 ${cleanId.slice(-6)}` : `工作流 ${cleanId.slice(-6)}`;
}
function normalizeRhEntries(values, kind){
    ...
    return (Array.isArray(values) ? values : []).map(raw => {
        ...
        seen.add(id);
        const fallback = rhEntryFallbackTitle(kind, id);
        const entry = {
            id,
...
function parseRunningHubRunRef(value){
    ...
    // RunningHub 分享页使用 /workflow/<id>，执行路径使用 /run/workflow/<id>。
...
function normalizeRhPastedTitle(value, parsed){
    const title = String(value || '').replace(/\s+/g, ' ').trim().slice(0, 160);
    if(!title || !parsed) return '';
    if(title === parsed.id || title.toLowerCase() === 'runninghub') return '';
    if(title === rhEntryFallbackTitle(parsed.type, parsed.id)) return '';
    if(new RegExp(`^工作流\\s+${String(parsed.id).replace(/[.*+?^${}()|[\\]\\\\]/g, '\\\\$&')}$`, 'i').test(title)) return '';
    return title;
}
function extractRhClipboardTitle(html, parsed){
    if(!html || !parsed || typeof DOMParser === 'undefined') return '';
    try {
        const doc = new DOMParser().parseFromString(String(html), 'text/html');
        const links = Array.from(doc.querySelectorAll('a[href]'));
        let matchedLink = false;
        for(const link of links){
            const ref = parseRunningHubRunRef(link.getAttribute('href') || '');
            if(ref && ref.type === parsed.type && ref.id === parsed.id){
                matchedLink = true;
                const title = normalizeRhPastedTitle(link.textContent, parsed);
                if(title) return title;
            }
        }
        return matchedLink ? normalizeRhPastedTitle(doc.title, parsed) : '';
    } catch(_) {
        return '';
    }
}
...
function handleRhPasteInput(value){
    ...
    if(String(value || '').trim() !== rhPasteMeta.value) rhPasteMeta = {value:'', title:''};
    if(parsed) setStatus('已识别 RunningHub 链接，点击右侧创建卡片');
}
async function hydrateRhWorkflowTitle(workflowId, initialTitle){
    const cleanId = String(workflowId || '').trim();
    if(!cleanId || rhTitleHydrationIds.has(cleanId) || rhTitleHydrationAttemptedIds.has(cleanId)) return;
    rhTitleHydrationAttemptedIds.add(cleanId);
    rhTitleHydrationIds.add(cleanId);
    try {
        const {response, data:payload} = await requestJson(
            `/api/runninghub/workflow-info?workflowId=${encodeURIComponent(cleanId)}`,
            undefined,
            '读取工作流名称失败'
        );
        const title = normalizeRhPastedTitle(payload?.data?.title || payload?.data?.workflowName || payload?.data?.name, {type:'workflow', id:cleanId});
        if(!response.ok || !title) return;
        const item = provider();
        if(!item || item.id !== 'runninghub') return;
        ensureRunningHubLists(item);
        const entry = item.rh_workflows.find(candidate => String(candidate.workflowId || candidate.id || '') === cleanId);
        if(!entry || entry.title !== initialTitle || entry.title !== rhEntryFallbackTitle('workflow', cleanId)) return;
        entry.title = title;
        renderRunningHubCards();
        const saved = await saveProviders();
        if(saved) setStatus(`已解析工作流名称：${title}`);
    } catch(_) {
        // 名称查询失败不影响已创建的卡片，保留 ID 降级名称。
    } finally {
        rhTitleHydrationIds.delete(cleanId);
    }
}
function hydrateRhFallbackWorkflowTitles(item=provider()){
    if(!item || item.id !== 'runninghub') return;
    ensureRunningHubLists(item);
    item.rh_workflows
        .filter(entry => entry?.hidden !== true && entry.title === rhEntryFallbackTitle('workflow', entry.workflowId || entry.id))
        .forEach(entry => hydrateRhWorkflowTitle(entry.workflowId || entry.id, entry.title));
}
async function createRhEntryFromPaste(){
    const item = provider();
    if(!item || item.id !== 'runninghub') return;
    const parsed = parseRunningHubRunRef(rhPasteInput?.value || '');
    if(!parsed){ setStatus('请粘贴 RunningHub 的 /workflow/...、/run/workflow/... 或 /run/ai-app/... 链接'); return; }
    ensureRunningHubLists(item);
    ...
    const pastedTitle = rhPasteMeta.value === String(rhPasteInput?.value || '').trim()
        ? normalizeRhPastedTitle(rhPasteMeta.title, parsed)
        : '';
    if(existingIndex >= 0 && item[listKey][existingIndex]?.hidden === true){
        ...
        item[listKey].unshift({
            ...
            title:pastedTitle || rhEntryFallbackTitle(parsed.type, parsed.id),
            note:'',
            thumbnail:'',
            enabled:true
        });
    ...
    rhPasteMeta = {value:'', title:''};
    renderRunningHubCards();
    setStatus(exists ? '这个 RunningHub 项目已经存在' : '已创建 RunningHub 卡片，正在保存...');
    if(!exists){
        const ok = await saveProviders();
        setStatus(ok ? '已创建并保存 RunningHub 卡片' : '已创建 RunningHub 卡片，但自动保存失败');
        if(ok && parsed.type === 'workflow' && !pastedTitle) hydrateRhWorkflowTitle(parsed.id, rhEntryFallbackTitle('workflow', parsed.id));
    }
}
...
async function removeRhEntry(kind, index){
    ...
    if(kind === 'workflow' && entryId) rhTitleHydrationAttemptedIds.delete(entryId);
    ...
    if(kind === 'workflow' && entryId){
        try {
            await requestJson(`/api/runninghub/workflows/${encodeURIComponent(entryId)}`, {method:'DELETE'}, '删除工作流失败');
        } catch(_) {}
    }
    const ok = await saveProviders();
...
async function openRhWorkflowEditor(index){
    ...
    rhWorkflowEditorState = { open:true, index, entry, config:null, expanded:{}, activeNodeId:'', graph:{ k:1, x:0, y:0, w:0, h:0 }, pan:null, bound:false, previewParams:{}, previewRunning:false, previewStatus:'', previewOutputs:[], previewJobId:'', previewAssetIds:[] };
...
async function openRhAppEditor(index){
    ...
    if(!entry) return;
    rhEditorMode = 'app';
    rhWorkflowEditorState = { open:true, index, entry, config:null, expanded:{}, activeNodeId:'app', graph:{ k:1, x:0, y:0, w:0, h:0 }, pan:null, bound:false, previewParams:{}, previewRunning:false, previewStatus:'', previewOutputs:[], previewJobId:'', previewAssetIds:[] };
    if(rhWorkflowEditorOverlay) rhWorkflowEditorOverlay.classList.add('open');
...
async function loadRhWorkflowEditorConfig(entry){
    ...
    const existing = response.status === 404 ? null : (data.workflow || null);
    if(existing) {
        config = existing;
    } else {
...
async function fetchRhAppEditor(force=false){
    ...
    const {data} = await requestJson(
        `/api/runninghub/app-info?webappId=${encodeURIComponent(appId)}`,
        undefined,
        '拉取应用参数失败'
    );
    if(data.success === false) throw new Error(data.detail || '拉取应用参数失败');
...
async function fetchRhWorkflowEditor(force=false){
    ...
    if(!workflowId) throw new Error('workflowId 为空');
    if(force) renderRhWorkflowEditorLoading('正在重新拉取...');
    const {data} = await requestJson('/api/runninghub/workflows/fetch', {
        method:'POST',
    ...
    if(data.success === false) throw new Error(data.detail || '拉取工作流失败');
...
function renderRhNodePopover(nodeId, anchorEl){
    ...
    pop.innerHTML = `
        ...
        <div class="rh-popover-body" data-floating-content>${fields.map(field => renderRhWorkflowEditorField(field)).join('')}</div>
    `;
    document.body.appendChild(pop);
    window.FloatingDismissal?.mark(pop, {
        surface: 'api-node-popover',
        kind: 'popover',
        content: '.rh-popover-body',
        close: closeRhNodePopover,
    });
...
async function saveRhWorkflowEditor(){
    ...
    try {
        ...
        const {data} = await requestJson(`/api/runninghub/workflows/${encodeURIComponent(config.workflowId)}`, {
            method:'PUT',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({
        ...
        if(data.success === false) throw new Error(data.detail || '保存失败');
...
function renderRhMappedPreviewHtml(config){
    ...
    const outputsHtml = (rhWorkflowEditorState.previewOutputs || []).length
        ...
        : '';
    const jobId = String(rhWorkflowEditorState.previewJobId || '').trim();
    const assetIds = (rhWorkflowEditorState.previewAssetIds || []).map(value => String(value || '').trim()).filter(Boolean);
    const persistedHtml = jobId || assetIds.length
        ? `<div class="rh-preview-persisted"><strong>已登记到统一资产库</strong><span>任务：${escapeHtml(jobId || '—')}</span>${assetIds.length ? `<span>资产：${assetIds.map(escapeHtml).join('、')}</span>` : '<span>资产：等待登记</span>'}<div class="rh-preview-persisted-actions">${jobId ? `<a href="/static/task-center.html?job_id=${encodeURIComponent(jobId)}" target="_blank" rel="noopener">打开任务中心</a>` : ''}${assetIds.length ? `<button type="button" onclick="openRhPreviewAsset('${escapeAttr(assetIds[0])}')">打开资产库</button>` : ''}</div></div>`
        : '';
...
function openRhPreviewAsset(assetId){
    const id = String(assetId || '').trim();
    if(!/^[A-Za-z0-9][A-Za-z0-9_.:@-]{0,191}$/.test(id)) return;
    if(window.parent !== window){
        window.parent.postMessage({type:'gw-workspace-open-asset', version:1, asset_id:id}, location.origin);
        return;
    }
    const destination = new URL('/static/asset-manager.html', location.origin);
    destination.searchParams.set('asset_id', id);
...
async function pickRhPreviewMedia(key, kind){
    ...
    input.onchange = async () => {
        ...
        try {
            const {data} = await requestJson('/api/ai/upload', {method:'POST', body:form}, '上传失败');
...
async function rhPreviewUploadValueIfNeeded(value){
    ...
    const {data} = await requestJson('/api/runninghub/upload-asset', {
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({url:text})
    }, 'RunningHub 素材上传失败');
    if(data.success === false) throw new Error(data.detail || data.error || 'RunningHub 素材上传失败');
...
async function testRhMappedPreview(){
    ...
    rhWorkflowEditorState.previewJobId = '';
    rhWorkflowEditorState.previewAssetIds = [];
    renderRhMappedPreview();
    try {
        ...
        const body = rhEditorMode === 'workflow'
            ? {workflowId:String(config.workflowId || '').trim(), nodeInfoList, track_job:true, ...(workflow ? {workflow} : {})}
            : {webappId:String(config.appId || '').trim(), nodeInfoList, track_job:true};
        ...
        const {data:submitData} = await requestJson(endpoint, {
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify(body)
        }, 'RunningHub 提交失败');
        if(submitData.success === false) throw new Error(submitData.detail || submitData.error || 'RunningHub 提交失败');
        ...
        if(!jobId) throw new Error('RunningHub 没有返回本地 job_id');
        rhWorkflowEditorState.previewJobId = jobId;
        rhWorkflowEditorState.previewStatus = `任务已提交：${taskId}`;
        renderRhMappedPreview();
        let result = null;
        for(let i = 0; i < 720; i++){
            await new Promise(resolve => setTimeout(resolve, 2500));
            const {data:queryData} = await requestJson(
                `/api/runninghub/query?taskId=${encodeURIComponent(taskId)}&job_id=${encodeURIComponent(jobId)}`,
                undefined,
                'RunningHub 查询失败'
            );
            if(queryData.success === false) throw new Error(queryData.detail || queryData.error || 'RunningHub 查询失败');
            const data = queryData.data || queryData;
        ...
        rhWorkflowEditorState.previewAssetIds = Array.isArray(result.asset_ids) ? result.asset_ids : [];
...
function openRhAppFieldPopover(key, anchorEl){
    ...
    pop.innerHTML = `
        ...
        <div class="rh-popover-body" data-floating-content>${renderRhWorkflowEditorField(field)}</div>
    `;
    document.body.appendChild(pop);
    window.FloatingDismissal?.mark(pop, {
        surface: 'api-app-popover',
        kind: 'popover',
        content: '.rh-popover-body',
        close: closeRhNodePopover,
    });
...
function renderRhEntryList(target, list, kind){
    if(!target) return;
    if(!list.length){
        target.innerHTML = `<div class="rh-empty">${kind === 'app' ? '粘贴 RunningHub /ai-app/... 或 /run/ai-app/... 后点击创建 AI 应用卡片' : '粘贴 RunningHub /workflow/... 或 /run/workflow/... 后点击创建工作流卡片'}</div>`;
        return;
    }
...
function renderEditor(){
    ...
    if(isRunningHub){
        ...
        renderRunningHubCards();
        hydrateRhFallbackWorkflowTitles(item);
    }
    if(isVolcengine){
...
window.onload = () => {
    ...
    if(rhPasteInput){
        rhPasteInput.addEventListener('paste', event => {
            const plain = String(event.clipboardData?.getData('text/plain') || '').trim();
            const html = event.clipboardData?.getData('text/html') || '';
            const parsed = parseRunningHubRunRef(plain);
            rhPasteMeta = {value:plain, title:extractRhClipboardTitle(html, parsed)};
        });
        rhPasteInput.addEventListener('input', () => handleRhPasteInput(rhPasteInput.value));
        rhPasteInput.addEventListener('keydown', event => {
            if(event.key !== 'Enter') return;
            event.preventDefault();
            createRhEntryFromPaste();
        });
    }
