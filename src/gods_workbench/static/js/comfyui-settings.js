...
function escapeJsString(s){
    return escapeHtml(JSON.stringify(String(s == null ? '' : s)).slice(1, -1).replace(/'/g, '\\u0027'));
}
async function fetchJsonOrThrow(url, options, fallbackMessage){
    ...
    if(!response.ok) throw new Error(detail || fallbackMessage);
    return data;
}
function workflowApiPath(name, suffix=''){
    const path = String(name || '').split('/').filter(Boolean).map(encodeURIComponent).join('/');
    return `/api/workflows/${path}${suffix}`;
}
...
async function loadComfyInstances(){
    try {
        const data = await fetchJsonOrThrow('/api/comfyui/instances', undefined, tr('comfy.loadFailed'));
...
async function saveComfyInstances(){
    ...
    setStatus('保存中...');
    try {
        const data = await fetchJsonOrThrow('/api/comfyui/instances', {
            method:'PUT',
...
async function loadList(){
    try {
        const data = await fetchJsonOrThrow('/api/workflows', undefined, tr('comfy.loadFailed'));
...
// iframe 由 V2 设置页嵌入，首次显示时可能 DOMContentLoaded 已经过去；
...
function renderList(){
    listEl.innerHTML = workflows.map(w => `
        <button class="workflow-card ${w.name===selectedName?'active':''}" type="button" onclick="selectWorkflow('${escapeJsString(w.name)}')">
            ...
            <span class="min-w-0" style="flex:1">
                ...
                <div class="workflow-meta">${escapeHtml(tf('comfy.fieldCount', {count:w.field_count}))}</div>
            </span>
...
async function selectWorkflow(name){
    ...
    try {
        ...
        const data = await fetchJsonOrThrow(workflowApiPath(name), undefined, tr('comfy.openFailed'));
    ...
    nodeListEl.innerHTML = nodes.map(([nodeId, node])=>{
        ...
        return `
            <div class="node-card ${expanded?'expanded':''}" id="node-card-${escapeAttr(nodeId)}" data-node-id="${escapeAttr(nodeId)}">
                <button class="node-card-head" type="button" onclick="toggleNodeCard(this)" aria-expanded="${expanded}" aria-controls="node-inputs-${escapeAttr(nodeId)}">
                    <span style="display:flex;align-items:center;gap:12px;min-width:0;flex:1">
                        ...
                        <span style="min-width:0">
                            <span class="node-class">${escapeHtml(nodeLabel(node))}</span>
                            <span class="node-id">${escapeHtml(nodeSub(node))} · #${escapeHtml(nodeId)} · ${tf('comfy.configurableCount', {count:inputs.length})}</span>
                        </span>
                    </span>
                    <span style="display:flex;align-items:center;gap:10px;flex:0 0 auto">
                        <span class="node-stats">${exposedCount > 0 ? tf('comfy.exposedCount', {count:exposedCount}) : ''}</span>
                        <i data-lucide="chevron-down" class="w-4 h-4 node-chev"></i>
                    </span>
                </button>
                <div class="node-inputs" id="node-inputs-${escapeAttr(nodeId)}">
...
function renderGraph(){
    ...
    const nodesHtml = Object.entries(currentWorkflow).map(([id, node]) => {
        ...
        return `
            <g class="gnode cat-${cat} ${exposedClass}" data-node-id="${escapeAttr(id)}" transform="translate(${pos.x},${pos.y})" role="button" tabindex="0" aria-label="${escapeAttr(`${label} · ${sub}`)}" onclick="openNodePopup('${escapeJsString(id)}', this)" onkeydown="activateGraphNodeOnEnterOrSpace(event, '${escapeJsString(id)}', this)">
...
function activateGraphNodeOnEnterOrSpace(event, nodeId, nodeElement){
    if(event.key !== 'Enter' && event.key !== ' ') return;
    event.preventDefault();
    openNodePopup(nodeId, nodeElement);
}
...
function openNodePopup(nodeId, gEl){
    ...
    popup.innerHTML = `
        <div class="popup-head">
            ...
            <button class="popup-close" type="button" onclick="closeNodePopup()" aria-label="${escapeAttr(tr('common.close') || '关闭')}"><i data-lucide="x" class="w-4 h-4"></i></button>
        </div>
...
function toggleNodeList(){
    ...
    document.getElementById('nodesToggle')?.setAttribute('aria-expanded', String(!hidden));
...
function renderInputRow(nodeId, inputKey, rawValue){
    ...
    const safeNodeId = escapeJsString(nodeId);
    const safeInputKey = escapeJsString(inputKey);
    const safeFieldId = active ? escapeJsString(f.id) : '';
    return `
        <div class="input-row ${active?'is-active':''} ${showExtras?'has-extras':''}">
            <button class="check-toggle ${active?'checked':''}" type="button" onclick="toggleField('${safeNodeId}','${safeInputKey}')" aria-pressed="${active}" aria-label="${escapeAttr(friendlyName)}">
            ...
            <input class="small-input" type="text" placeholder="${tr('comfy.displayName')}" value="${active?escapeAttr(f.name):escapeAttr(friendlyName)}" ${active?'':'disabled'} oninput="updateField('${safeFieldId}','name',this.value)">
            <select class="small-select" ${active?'':'disabled'} onchange="updateField('${safeFieldId}','type',this.value)">
...
function renderExtras(f){
    if(f.type === 'slider' || f.type === 'number'){
        const safeFieldId = escapeJsString(f.id);
        const randomToggle = f.type === 'number'
            ? `<label class="random-toggle" onclick="event.stopPropagation()"><input type="checkbox" ${f.random_enabled === true ? 'checked' : ''} onchange="updateField('${safeFieldId}','random_enabled',this.checked)">随机数</label>`
            : '';
        return `<div class="extras-row">
            <div class="extra-pair">min<input class="small-input" type="number" value="${escapeAttr(f.min ?? '')}" oninput="updateField('${safeFieldId}','min',this.value===''?null:parseFloat(this.value))"></div>
            <div class="extra-pair">max<input class="small-input" type="number" value="${escapeAttr(f.max ?? '')}" oninput="updateField('${safeFieldId}','max',this.value===''?null:parseFloat(this.value))"></div>
            <div class="extra-pair">step<input class="small-input" type="number" value="${escapeAttr(f.step ?? '')}" oninput="updateField('${safeFieldId}','step',this.value===''?null:parseFloat(this.value))"></div>
            <div class="extra-pair">${tr('comfy.defaultValue')}<input class="small-input" type="number" value="${escapeAttr(f.default ?? '')}" oninput="updateField('${safeFieldId}','default',this.value===''?null:parseFloat(this.value))"></div>
            ${randomToggle}
        </div>`;
    }
    if(f.type === 'dropdown'){
        const opts = f.options || [];
        const fid = escapeJsString(f.id);
...
function setPreviewValue(fieldId, value){
    ...
    // 更新滑块旁边的数值显示
    const valSpan = [...document.querySelectorAll('[data-slider-val]')].find(el => el.dataset.sliderVal === String(fieldId));
...
function togglePreviewBoolean(fieldId, control){
    const enabled = control.getAttribute('aria-checked') !== 'true';
    setPreviewValue(fieldId, enabled);
...
function randomButtonHtml(f){
    ...
    return `<button class="random-btn ${active ? 'active' : ''}" type="button" onclick="togglePreviewRandom('${escapeJsString(f.id)}')" title="${escapeAttr(title)}"><i data-lucide="dice-5" class="w-4 h-4"></i></button>`;
}
...
function renderPreview(){
    ...
    const resultHtml = runResult
        ? `<div class="run-result"><img src="${escapeAttr(runResult)}" onclick="openImagePreview('${escapeJsString(runResult)}')"><div class="run-status">${tr('comfy.runSuccess')}</div></div>`
        : '';
...
function renderPreviewField(f){
    ...
    const safeFieldId = escapeJsString(f.id);
    if(f.type === 'textarea'){
        return `<div class="pfield">${label}<textarea class="pfield-textarea" oninput="setPreviewValue('${safeFieldId}',this.value)">${escapeHtml(v)}</textarea></div>`;
    }
    if(f.type === 'number'){
        const randomBtn = randomButtonHtml(f);
        return `<div class="pfield">${label}<div class="pfield-random-row" style="${randomBtn ? '' : 'grid-template-columns:1fr'}"><input class="pfield-input" type="number" value="${escapeAttr(v)}" oninput="setPreviewValue('${safeFieldId}',parseFloat(this.value)||0)">${randomBtn}</div></div>`;
    }
    if(f.type === 'slider'){
        ...
        return `<div class="pfield">${label}<div class="pfield-random-row" style="grid-template-columns:1fr"><div class="pfield-slider">
            <input type="range" min="${escapeAttr(min)}" max="${escapeAttr(max)}" step="${escapeAttr(step)}" value="${escapeAttr(v)}" oninput="setPreviewValue('${safeFieldId}',parseFloat(this.value))">
            <span class="pfield-slider-val" data-slider-val="${escapeAttr(f.id)}">${escapeHtml(v)}</span>
        </div></div></div>`;
    }
    if(f.type === 'dropdown'){
        const opts = (f.options || []).map(o => `<option value="${escapeAttr(o)}" ${String(v)===String(o)?'selected':''}>${escapeHtml(o)}</option>`).join('');
        return `<div class="pfield">${label}<select class="pfield-select" onchange="setPreviewValue('${safeFieldId}',this.value)">${opts || `<option value="">${tr('comfy.noOptions')}</option>`}</select></div>`;
    }
    if(isMediaField(f)){
        ...
        return `<div class="pfield">${label}<div class="pfield-image-drop ${displayUrl?'has-image':''}" role="button" tabindex="0" aria-label="${escapeAttr(mediaUploadText(fieldKind(f)))}" onclick="pickImage('${safeFieldId}')" onkeydown="activateOnEnterOrSpace(event)">
            ${mediaPreviewHtml(fieldKind(f), displayUrl, v)}
        </div></div>`;
    }
    if(f.type === 'boolean'){
        return `<div class="pfield">${label}<div class="pfield-bool">
            <button class="pfield-bool-track ${v?'on':''}" type="button" role="switch" aria-checked="${!!v}" aria-label="${escapeAttr(f.name || f.input)}" onclick="togglePreviewBoolean('${safeFieldId}', this)">
                <div class="pfield-bool-thumb"></div>
            </button>
        </div></div>`;
    }
    return `<div class="pfield">${label}<input class="pfield-input" type="text" value="${escapeAttr(v)}" oninput="setPreviewValue('${safeFieldId}',this.value)"></div>`;
}
...
function renderMiniCanvasPreview(target = previewCard, large = false){
    ...
    const resultHtml = runResult
        ? `<div class="mini-result"><img src="${escapeAttr(runResult)}" onclick="openImagePreview('${escapeJsString(runResult)}')"><div class="run-status">${tr('comfy.runSuccess')}</div></div>`
    ...
    target.innerHTML = `
        <div id="miniCanvas" class="mini-canvas ${large ? 'large' : ''}">
            ...
            <div id="miniWorld" class="mini-world" style="transform:translate(${miniView.x}px,${miniView.y}px) scale(${miniView.k})">
                ...
                ${prompts.map((n,i) => `
                    <div class="mini-card" data-node="${escapeAttr(n.id)}" style="left:${n.x}px;top:${n.y}px">
                        ...
                        <div class="mini-card-body"><textarea class="mini-textarea" oninput="updateMiniNode('${escapeJsString(n.id)}','text',this.value)" placeholder="${escapeAttr(tr('comfy.promptPlaceholder'))}">${escapeHtml(n.text || '')}</textarea></div>
                    </div>`).join('')}
                ${mediaNodes.map((n,i) => `
                    <div class="mini-card" data-node="${escapeAttr(n.id)}" style="left:${n.x}px;top:${n.y}px">
                        <span class="mini-port out"></span>
                        ...
                        <div class="mini-card-body"><div class="mini-image-drop" role="button" tabindex="0" aria-label="${escapeAttr(mediaUploadText(n.type))}" onclick="pickMiniImage('${escapeJsString(n.id)}')" onkeydown="activateOnEnterOrSpace(event)">${mediaPreviewHtml(n.type, n.url, n.name || n.value, true)}</div></div>
                    </div>`).join('')}
                <div class="mini-card comfy-card" data-node="${escapeAttr(comfy.id)}" style="left:${comfy.x}px;top:${comfy.y}px">
                ...
                <div class="mini-card" data-node="${escapeAttr(output.id)}" style="left:${output.x}px;top:${output.y}px">
...
function renderMiniField(f){
    ...
    if(isMediaField(f)){
        const displayUrl = previewImageUrls[f.id] || (typeof v === 'string' && /^(\/|https?:|blob:|data:)/.test(v) ? v : '');
        return `<div class="pfield">${label}<div class="mini-image-drop" role="button" tabindex="0" aria-label="${escapeAttr(mediaUploadText(fieldKind(f)))}" onclick="pickImage('${safeFieldId}')" onkeydown="activateOnEnterOrSpace(event)">${mediaPreviewHtml(fieldKind(f), displayUrl, v, true)}</div></div>`;
    }
    if(f.type === 'textarea'){
        return `<div class="pfield">${label}<textarea class="mini-textarea" oninput="setPreviewValue('${safeFieldId}',this.value)">${escapeHtml(v)}</textarea></div>`;
    }
    if(f.type === 'number'){
        const randomBtn = randomButtonHtml(f);
        return `<div class="pfield">${label}<div class="pfield-random-row" style="${randomBtn ? '' : 'grid-template-columns:1fr'}"><input class="mini-input" type="number" value="${escapeAttr(v)}" oninput="setPreviewValue('${safeFieldId}',parseFloat(this.value)||0)">${randomBtn}</div></div>`;
    }
    if(f.type === 'slider'){
        const min = f.min ?? 0, max = f.max ?? 10, step = f.step ?? 1;
        return `<div class="pfield">${label}<div class="pfield-random-row" style="grid-template-columns:1fr"><div class="pfield-slider">
    ...
    if(f.type === 'boolean'){
        return `<div class="pfield">${label}<div class="pfield-bool"><button class="pfield-bool-track ${v?'on':''}" type="button" role="switch" aria-checked="${!!v}" aria-label="${escapeAttr(f.name || f.input)}" onclick="togglePreviewBoolean('${safeFieldId}', this)"><div class="pfield-bool-thumb"></div></button></div></div>`;
    }
    return `<div class="pfield">${label}<input class="mini-input" type="text" value="${escapeAttr(v)}" oninput="setPreviewValue('${safeFieldId}',this.value)"></div>`;
}

function miniDeleteButton(node){
    return ['prompt','image'].includes(node.type) ? `<button class="mini-delete" type="button" onclick="removeMiniNode('${escapeJsString(node.id)}')" title="${escapeAttr(tr('common.delete'))}"><i data-lucide="x" class="w-3 h-3"></i></button>` : '';
}
...
async function pickMiniImage(nodeId){
    ...
    input.onchange = async () => {
        ...
        try {
            const data = await fetchJsonOrThrow('/api/upload', { method:'POST', body:form }, mediaUploadFailedText(node.type));
...
async function pickImage(fieldId){
    ...
    input.onchange = async () => {
        ...
        form.append('files', file);
        try {
            const data = await fetchJsonOrThrow('/api/upload', { method:'POST', body:form }, mediaUploadFailedText(kind));
...
async function onRun(){
    ...
    try {
        ...
        const promptField = (currentConfig.fields || []).find(f => fieldKind(f) === 'prompt');
        const prompt = promptField ? String(runFields[promptField.id] ?? '') : '';
        const data = await fetchJsonOrThrow(workflowApiPath(selectedName, '/run'), {
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({ fields:runFields, config:currentConfig, client_id:'workflow-test', prompt })
        }, tr('comfy.runFailed'));
...
async function onUpload(event){
    ...
    try {
        ...
        if(!inputName) return;
        const result = await fetchJsonOrThrow('/api/workflows', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
...
async function onSave(){
    ...
    try {
        await fetchJsonOrThrow(workflowApiPath(selectedName, '/config'), {
            method:'PUT',
            headers:{'Content-Type':'application/json'},
...
async function onDelete(){
    ...
    try {
        await fetchJsonOrThrow(workflowApiPath(selectedName), { method:'DELETE' }, tr('comfy.deleteFailed'));
        selectedName = '';
