...
- 原报告 §D 第 60 行 / §E 第 69 行「旧集成标记用例登记为『专项任务，尚未完成』」：**已完成**——T13 由本地提交 `28c23bf` + `996c3ba` 落地（基线重定义 + 移除快捷工具/画布内页），全量 `pytest` **40 passed**（本地）。
- 原报告 P1（`.git` 内嵌二进制，见第 68 行 E15 与 §三 P1 第 89 行）：**已完成**——T12 于 2026-09-19 获用户明确授权并执行（仓外 bundle 备份 + 引用清理后仅剩 3 个白名单 `.otf`）。
- 上述两处「尚未完成」表述现已过时；原文保留不改。
- 指针：`TASK-NOTES-2026-09-18.md` §2（T12）、§4.1（T13）。
- 证据边界：均为**本地**门禁与**本地**提交；**未 push、未跑远端 CI、未做生产验收**，**不等于生产就绪**。
