...
P8-A1 报告 §6 自述「计数为静态下界近似」。本轮 E2E 反向暴露出扫描器的两个**独立缺陷**（现均已修正，
...
2. **未修改 P8-A1 的扫描器与冻结基线**（§5.4 的三类偏差**仅登记**）。
...
2. P8-A1 扫描器剩余偏差（helper 拼接类，如 `/api/canvases/{id}/meta|touch|purge`）是否**立即单独立项修复**（§5.4）；缺陷 A/B 已修复。
...
| 远端 CI | **本轮未执行**（推送与 CI 读回由主代理在同批提交后完成） |
| 生产验收 | **未执行**，仍为独立决策 |
