...
1. **契约方法全量对齐（0 缺失）**：
   ...
   - **比对结果**：契约共 **51** 个方法条目，全部在 `app.openapi()` 导出的 73 条路由中 100% 存在，**缺失数 = 0**。（注：该基数经主代理复核更正为 **72**，见 §6.3；此处保留初稿原值不改写。）
...
本节由主代理 `/root` 在正式入库前追加，用于更正本报告初稿的三处口径问题，并登记实测复核证据。

1. **API 引用计数：188 → 189（口径更正）**：
   - 本报告初稿与 `TASKS.md` T81 原写「前端 188 引用 / 140 缺口」。
   - 主代理以 T46 已裁决的**唯一权威口径**（守卫 `tests/contracts/test_phase8_frontend_backend_api_gap.py`）实测复算：
     ...
     现场调用守卫内部扫描函数得 `front=189`、`backend=55`、`impl=49`、`unimpl=140`，与常量逐字一致。
   - 故将本报告与 T81 中的「188」更正为 **189**。
   - 注意：`HANDOFF-8.md` §8.1 的 `188 / 14 / 8 / 180` 是**另一套口径**（另一修复阶段的扫描器基线），
     与本守卫口径不可直接相减；两者并存不矛盾，但**当前引用统一以守卫为准**。
2. **独立性口径更正（本报告初稿称「第三方洁净室独立审计」，实际执行主体为同框架代理）**：
...
3. **`app.openapi()` 路由基数：73 → 72（基数更正）**：
   - 原文 §3.1 写「全部在 `app.openapi()` 导出的 **73** 条路由中 100% 存在」。
   - 主代理以 `from gods_workbench.api.app import app` 实测枚举 `app.openapi()['paths']`：
     **method+path 操作数 = 72**（unique path = 54），无任何非 method 键混入。
   - 结论：**基数应为 72**。原文的「73」与「51 个方法条目全部 100% 存在」这一**核心结论不受影响**
     （51 个方法条目均为 72 条路由的子集，仍然成立；缺失数仍为 0）；本项仅为基数笔误更正。
   - 另注：守卫 `test_phase8_frontend_backend_api_gap.py` 的口径是**唯一路由路径 = 55**（method+path = 75），
     与 OpenAPI 的 72 是**两套不同工具、不同统计对象**（守卫扫描 `APIRouter` 装饰器；OpenAPI 走 schema 导出），
     二者不可直接互换引用。
